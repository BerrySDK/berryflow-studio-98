...
import { Smartphone, QrCode, RefreshCw, Power, Plus, Link2, Loader2 } from "lucide-react";
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button className="bg-gradient-primary shadow-glow">
                <Plus className="mr-1.5 h-4 w-4" /> Nova sessao
              </Button>
            </DialogTrigger>
      ...
      <div className="space-y-6 p-8">
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {(sessions.data ?? []).map((session) => (
            <Card key={session.id} className="bg-gradient-surface p-5">
              <div className="flex items-center gap-3">
                <div className="grid h-11 w-11 place-items-center rounded-lg bg-primary/10 ring-1 ring-primary/30">
                  <Smartphone className="h-5 w-5 text-primary-glow" />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-base font-semibold">{session.name}</p>
                  <p className="text-xs text-muted-foreground">{session.phoneNumber ?? "Sem numero pareado"}</p>
                </div>
              ...
              {(session.qrCode || session.pairingCode) && session.status !== "connected" ? (
                <div className="mt-4 rounded-lg border border-primary/30 bg-primary/5 p-3 text-xs text-primary-glow">
              ...
              <div className="mt-4 flex gap-2">
                {session.status === "disconnected" ? (
                  <Button
                    ...
                    onClick={() => conn.mutate(session.id)}
                  ...
                  <>
                    <Button
                      ...
                      variant="outline"
                      className="flex-1"
                      onClick={() => reconn.mutate(session.id)}
                      disabled={reconn.isPending}
                    >
                    ...
                    <Button
                      ...
                      variant="outline"
                      className="flex-1"
                      onClick={() => disc.mutate(session.id)}
...
function ConnectionPanel({ session }: { session: Session }) {
  ...
  return (
    <Card className="bg-gradient-surface p-5">
      ...
      <div className="grid gap-4 lg:grid-cols-[260px,1fr]">
        <div className="grid place-items-center rounded-xl border border-primary/30 bg-card/80 p-4">
          {session.qrCode ? (
            qrDataUrl ? (
              <img src={qrDataUrl} alt={`QR da sessao ${session.name}`} className="h-60 w-60 rounded-lg" />
            ) : (
              <div className="grid h-60 w-60 place-items-center rounded-lg bg-primary/5">
                <Loader2 className="h-6 w-6 animate-spin text-primary-glow" />
              </div>
            )
          ) : (
            <div className="grid h-60 w-60 place-items-center rounded-lg bg-primary/5 text-center">
              <div>
        ...
        <div className="space-y-3 rounded-xl border border-border bg-card/40 p-4">
          ...
          <div className="rounded-lg border border-border bg-background/80 p-3 text-xs text-muted-foreground">
            <p>Sessao: <span className="font-mono text-foreground">{session.id}</span></p>
            {session.phoneNumber ? <p>Numero: <span className="font-mono text-foreground">{session.phoneNumber}</span></p> : null}
          </div>
        </div>
      </div>
    </Card>
